...
def load_old_pretrain_file(net, pretrain_file, skip=[]):
    ...
    for key in keys:
        ...
        if 'center.0.' in key:
            key0 = key.replace('center.0.','center.block.1.conv.')
            state_dict[key] = pretrain_state_dict[key0]
            continue

        if 'center.2.' in key:
            key0 = key.replace('center.2.','center.block.2.conv.')
            state_dict[key] = pretrain_state_dict[key0]
            continue
        ...
        for i in range(5,0,-1):
            if 'decoder%d.conv1.'%i in key:
                key0 = key.replace('decoder%d.conv1.'%i ,'dec%d.block.1.conv.'%i )
                state_dict[key] = pretrain_state_dict[key0]
                found=1
            if 'decoder%d.conv2.'%i in key:
                key0 = key.replace('decoder%d.conv2.'%i ,'dec%d.block.2.conv.'%i )
                state_dict[key] = pretrain_state_dict[key0]
                found=1
        if found==1:continue
...
def run_train():
    ...
    out_dir = '/root/share/project/kaggle/tgs/results/resnet34-256/fold0-1a'
    initial_checkpoint = \
        '/root/share/project/kaggle/tgs/results/resnet34-256/fold0-1/checkpoint/00002500_model.pth'
